package com.cyx.zsfxpt.auth.jwt;
...
public class JwtService {
    ...
    private final JwtEncoder jwtEncoder;//jwt编码器
    private final JwtDecoder jwtDecoder;//jwt解码器
    private final AuthProperties properties;//jwt配置项
    private final Clock clock = Clock.systemUTC();//时钟对象
    ...
    public TokenPair issueTokenPair(User user) {
        String refreshTokenId = UUID.randomUUID().toString();//用uuid来作为刷新令牌id
        Instant issuedAt = Instant.now(clock);//获得当前时间
        Instant accessExpiresAt = issuedAt.plus(properties.getJwt().getAccessTokenTtl());//获得访问令牌的过期时间
        Instant refreshExpiresAt = issuedAt.plus(properties.getJwt().getRefreshTokenTtl());//获得刷新令牌的过期时间

        //编码一个访问令牌，令牌类型是access
    ...
    private String encodeToken(User user, Instant issuedAt, Instant expiresAt, String tokenType, String tokenId) {
        JwtClaimsSet claims = JwtClaimsSet.builder()
                .issuer(properties.getJwt().getIssuer())//设置一个jwt签发者的标识，就是一个普通的标识，这里是“zhiguang”
                .issuedAt(issuedAt)//当前时间
                .expiresAt(expiresAt)//访问令牌刷新时间
                .subject(String.valueOf(user.getId()))//用户id
                .id(tokenId)//访问令牌id
                .claim(CLAIM_TOKEN_TYPE, tokenType)//令牌类型
                .claim(CLAIM_USER_ID, user.getId())//用户id
                .claim("nickname", user.getNickname())//用户昵称，自己起的名字
                .build();
        return jwtEncoder.encode(JwtEncoderParameters.from(claims)).getTokenValue();//将这些信息编进jwt中，生成编码
    }
    ...
    public long extractUserId(Jwt jwt) {
        ...
        if (claim instanceof Number number) {//number类是java中所有数字类型类的父类
