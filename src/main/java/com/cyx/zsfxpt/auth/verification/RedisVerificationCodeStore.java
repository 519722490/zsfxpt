...
public class RedisVerificationCodeStore implements VerificationCodeStore {
    ...
    public void saveCode(String scene, String identifier, String code, Duration ttl, int maxAttempts) {
        String key = buildKey(scene, identifier);//场景加手机号组成key
        HashOperations<String, String, String> ops = redisTemplate.opsForHash();//构造一个redis的hash结构
        try {
            ops.put(key, FIELD_CODE, code);//存放验证码
            ops.put(key, FIELD_MAX_ATTEMPTS, String.valueOf(maxAttempts));//存放最大尝试次数
            ops.put(key, FIELD_ATTEMPTS, "0");//存放当前已经输错了几次
            redisTemplate.expire(key, ttl);//设置一个过期时间
    ...
    public VerificationCheckResult verify(String scene, String identifier, String code) {
        ...
        if (data.isEmpty()) {//为空返回验证码为空
            return new VerificationCheckResult(VerificationCodeStatus.NOT_FOUND, 0, 0);
        }
        String storedCode = data.get(FIELD_CODE);//获得验证码
        int maxAttempts = parseInt(data.get(FIELD_MAX_ATTEMPTS), 5);//获得最大重试次数
        int attempts = parseInt(data.get(FIELD_ATTEMPTS), 0);//获得当前重试了几次

        if (attempts >= maxAttempts) {//超过最大重试次数就返回错误结果
            return new VerificationCheckResult(VerificationCodeStatus.TOO_MANY_ATTEMPTS, attempts, maxAttempts);
        }

        if (Objects.equals(storedCode, code)) {//验证码正确就删除这个验证码，然后返回验证成功
            redisTemplate.delete(key);//验证码校验成功后会立即删除。这样防止同一个验证码被重复使用。
        ...
        if (updatedAttempts >= maxAttempts) {
            redisTemplate.expire(key, Duration.ofMinutes(30));//会把这个验证码 key 再保留 30 分钟。目的是短时间内别让这个验证码继续被尝试
